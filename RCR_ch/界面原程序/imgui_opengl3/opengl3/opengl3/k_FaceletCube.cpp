...
void FaceletCube::SetFaceMarkings(int face, std::string markings) {
    int facelet;
...
int FaceletCube::FaceNameToOffset(char faceName) {
    ...
    char *name{ strchr(faceNames, faceName) };
    if (!name)
        return -1;
    return static_cast<int>(name - faceNames); // Compute face enumeration
}

int FaceletCube::FaceletOffsetToMarking(int offset) {
    char* name{ strchr(faceMarkings, faceletMarkings[offset]) };
    if (!name)
        return -1;
    return static_cast<int>(name - faceMarkings);	// Compute face enumeration
}
...
std::string FaceletCube::ErrorText(unsigned int error) {
    if (error >= NumberOfErrors) error = 0;
    return errorText[error];
}

std::string FaceletCube::errorText[NumberOfErrors] = {
    "",
