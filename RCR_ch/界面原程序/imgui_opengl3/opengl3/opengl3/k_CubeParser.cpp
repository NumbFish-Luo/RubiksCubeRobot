...
int CubeParser::parseFacelets(std::vector<std::string>& faceletStrings, FaceletCube &faceletCube) {
    int face;
    int faces[NUM_FACES];
    int status;
...
int CubeParser::parseFace(FaceletCube &faceletCube, std::string faceString, int &face) {
    int facelet;
...
// Return the text associated with an error return code
std::string CubeParser::ErrorText(unsigned int error) {
    if (error >= NumberOfErrors) error = 0;
    return errorText[error];
}

std::string CubeParser::errorText[NumberOfErrors] = {
    "",
